...
class Diffusion(L.LightningModule):
  ...
  def on_train_start(self):
    ...
    for dl in self.trainer.fit_loop._combined_loader.flattened:
      ...
      # Import the safe_collate_fn from dataloader module
      from dataloader import safe_collate_fn
      updated_dls.append(
  ...
  def forward(self, x, sigma):
    ...
    # Decode and print DNA sequences for debugging
    if hasattr(self, 'tokenizer') and x.numel() > 0:
        print(f"\n=== Forward Pass - Decoded Sequences ===")
        print(f"Batch shape: {x.shape}")
        
        # Print up to 3 sequences from the batch
        num_to_print = min(3, x.shape[0])
        ...
        for i in range(num_to_print):
            ...
            try:
                ...
                # Clean up special tokens and spaces
                clean_seq = decoded.replace('[BOS]', '').replace('[EOS]', '').replace('[PAD]', '')
                clean_seq = ''.join(clean_seq.split())  # Remove all spaces
                
                # Count mask tokens if they exist
                mask_count = (sequence == self.mask_index).sum().item() if hasattr(self, 'mask_index') else 0
                total_tokens = len(sequence)
                
                # Display sequence (truncated if too long)
                display_seq = clean_seq[:80] + ('...' if len(clean_seq) > 80 else '')
                #end of the sequence (last 80 tokens)
                end_of_sequence = clean_seq[-80:]
                print(f"End of sequence: {end_of_sequence}")

                
                print(f"Seq {i+1:2d}: {display_seq}")
                print(f"       Length: {len(clean_seq)}, Masks: {mask_count}/{total_tokens}")
                
                # Show first 10 raw token IDs
                raw_tokens = sequence[:10].tolist()
                print(f"       Raw IDs: {raw_tokens}")
                
            except Exception as e:
                print(f"Seq {i+1:2d}: <decoding error: {e}>")
  ...
  def _ddpm_update(self, x, t, dt, return_process=False):
    # Store original x for return_process
    original_x = x
    
    # Convert continuous representation back to indices if needed
    if x.ndim == 3 and x.shape[-1] == self.vocab_size:
      # x is already one-hot encoded, keep it for processing
      x_one_hot = x
      x = x.argmax(dim=-1)  # Convert to indices for forward pass
    elif x.ndim == 2:
      # x is indices, convert to one-hot for processing
    ...
    log_p_x0 = self.forward(x, unet_conditioning)  # x is indices here
    ...
    copy_flag = (x != self.mask_index).to(x.dtype)
    result = copy_flag * x + (1 - copy_flag) * _x
    
    if return_process:
      # Return original_x (which might be one-hot) for consistency with _ddpm_update_finetune_gradient
      return result, original_x, unet_conditioning, move_chance_t, copy_flag
    else:
      return result
  ...
  def _ddpm_update_finetune_gradient(self, x, t, dt, copy_flag_temp, return_process=False):
    ...
    # Ensure x is in one-hot format for gradient computation
    if x.ndim == 2 or x.shape[-1] != self.vocab_size:
      x_one_hot = F.one_hot(x, num_classes=self.vocab_size).to(torch.float32)
      x_indices = x
    else:
      x_one_hot = x
      x_indices = x.argmax(dim=-1)  # Convert one-hot to indices for forward pass
    ...
    move_chance_s = move_chance_s[:, None, None]
    unet_conditioning = sigma_t
    log_p_x0 = self.forward(x_indices, unet_conditioning)  # Use indices for forward pass
    assert move_chance_t.ndim == log_p_x0.ndim
    
    # log_p_x0 has shape [batch, seq_len, vocab_size] including mask token
    # We need to handle mask token probability properly
    q_xs = log_p_x0.exp() * (move_chance_t - move_chance_s)
    
    # Since log_p_x0 includes all vocab including mask, we can set mask probability
    ...
    if copy_flag_temp is not None:
      ...
      if x_one_hot.shape[-1] > self.mask_index:
        copy_flag_prob = 1 - x_one_hot[:, :, self.mask_index].unsqueeze(-1)
      else:
        # If mask_index is out of bounds, use zeros (no masking)
        copy_flag_prob = torch.ones_like(x_one_hot[:, :, 0]).unsqueeze(-1)
      soft_copy_flag = torch.nn.functional.sigmoid(copy_flag_prob/copy_flag_temp)
    else:
      if x_one_hot.shape[-1] > self.mask_index:
        soft_copy_flag = 1 - x_one_hot[:, :, self.mask_index].unsqueeze(-1)
      else:
        soft_copy_flag = torch.ones_like(x_one_hot[:, :, 0]).unsqueeze(-1)

    if return_process:
      return soft_copy_flag * x_one_hot + (1 - soft_copy_flag) * _x, x_one_hot, unet_conditioning, move_chance_t, soft_copy_flag
    else:
      return soft_copy_flag * x_one_hot + (1 - soft_copy_flag) * _x
  ...
  def _sample_finetune_gradient(self, num_steps=None, eps=1e-5, eval_sp_size=None, copy_flag_temp=None):
    ...
    # Return the full vocabulary probabilities for downstream processing
    # The fine-tuning script will extract the DNA bases as needed
    return x, last_x_list, condt_list, move_chance_t_list, copy_flag_list
  
  @torch.no_grad()
  def _sample_infill(self,
                     observed_ids: torch.LongTensor,
                     observed_mask: torch.BoolTensor,
                     num_steps: int = None,
                     eps: float = 1e-5):
    """
    Arbitrary-span infill: keep observed positions fixed, generate others.
    observed_ids: (B, L)  real tokens where observed_mask==True, dummy otherwise
    observed_mask: (B, L)  True=fixed, False=to-sample
    """
    assert observed_ids.shape == observed_mask.shape
    ...
    # init: masked everywhere, then clamp observed to their given tokens
    x = self._sample_prior(B, L).to(device)
    x = x.masked_scatter(observed_mask, observed_ids[observed_mask])
    ...
    for i in range(num_steps):
      ...
      # take one reverse step with your chosen sampler
      if self.sampler == 'ddpm':
        x_next = self._ddpm_update(x, t, dt)
      elif self.sampler == 'ddpm_cache':
        p_x0_cache, x_next = self._ddpm_caching_update(x, t, dt, p_x0=p_x0_cache)
        # if anything external changed (we also clamp), drop cache to be safe
        if (not torch.allclose(x_next, x)) or self.time_conditioning:
          p_x0_cache = None
      else:
        x_next = self._analytic_update(x, t, dt)

      # clamp observed positions so they never change
      x = torch.where(observed_mask, observed_ids, x_next)

    # optional “noise removal” pass
    if self.config.sampling.noise_removal:
      ...
      # re-clamp after the final denoise
      x = torch.where(observed_mask, observed_ids, x)

    return x

  def restore_model_and_infill(self,
                               observed_ids: torch.LongTensor,
                               observed_mask: torch.BoolTensor,
                               num_steps: int,
                               eps: float = 1e-5):
    """EMA-safe wrapper for _sample_infill (mirrors restore_model_and_sample)."""
    if self.ema:
    ...
    samples = self._sample_infill(observed_ids=observed_ids,

...
def _group_texts(examples, block_size, bos, eos):
  ...
  print(f"[DEBUG] _group_texts: block_size={block_size}, total_length={total_length}")
  print(f"[DEBUG] _group_texts: num_sequences={len(examples['input_ids'])}")
  ...
  print(f"[DEBUG] _group_texts: new_block_size={new_block_size}")
  total_length = (total_length // new_block_size) * new_block_size
  print(f"[DEBUG] _group_texts: adjusted_total_length={total_length}")
  ...
  print(f"[DEBUG] _group_texts: created {len(_values)} sequences")
...
def get_dataset(
  ...
  if dataset_name == 'wikitext103':
    ...
    if utils.fsspec_exists(split_cache):
      ...
      dataset_dict = raw_ds.train_test_split(test_size=0.1, seed=42)
      dataset_dict.save_to_disk(split_cache)

    if mode in ['train']:
      dataset = dataset_dict['train']
    else:  # 'validation' or 'test'
      dataset = dataset_dict['test']
      #limit it to 64 samples
      dataset = dataset.select(range(64))
      print("[DEBUG] validation dataset, ", dataset)

    # rename 'seq' → 'text' so downstream tokenizer code works unchanged
    dataset = dataset.rename_column('full_sequence', 'text')
    ...
    if mode in ['train']:
      ...
      dataset = dataset_dict['test']
      #limit it to 256 samples to ensure enough total tokens for grouping
      dataset = dataset.select(range(min(256, len(dataset))))
      print("[DEBUG] validation dataset, ", dataset)

    # rename 'seq' → 'text' so downstream tokenizer code works unchanged
    dataset = dataset.rename_column('Sequence 3 UTR', 'text')
    print("[DEBUG] dataset, ", dataset)
  ...
  elif dataset_name == 'ncbi_hm':
    # load the full CSV – always in **non-streaming** mode so we can split once
    ...
    # Only create the train/val split once and cache it on disk
    split_cache = os.path.join(cache_dir, 'ncbi_hm_split')
    if utils.fsspec_exists(split_cache):
      dataset_dict = datasets.load_from_disk(split_cache)
    else:
      dataset_dict = raw_ds.train_test_split(test_size=0.1, seed=42)
  ...
  if dataset_name in ['lambada', 'openwebtext-train',
                      'openwebtext-valid', 'ncbi_hm', 'ginkgo', 'ginkgo_utr', 'ginkgo_3utr']:
    data = dataset
  else:
    data = dataset[mode]
  ...
  def preprocess_and_tokenize(example):
    ...
    # Special handling for DNA sequences - separate handling for each dataset
    if dataset_name == 'ncbi_hm':
      print("[DEBUG] ncbi_hm text, ", text) 
      if isinstance(text, list):
        # Handle batched data for ncbi_hm
        processed_texts = []
        for seq in text:
          if seq:  # Check if sequence is not empty
            # Add spaces between characters for char_tokenizer
            processed_seq = ' '.join(list(seq)) + ' [SEP]'
            processed_texts.append(processed_seq)
        text = processed_texts
      else:
        # Handle single sequence for ncbi_hm
        text = ' '.join(list(text)) + ' [SEP]'
      
    elif dataset_name == 'ginkgo' or dataset_name == 'ginkgo_3utr':
      print("[DEBUG] ginkgo text, ", text) 
      if isinstance(text, list):
        # Handle batched data for ginkgo
        processed_texts = []
        for seq in text:
          if seq:  # Check if sequence is not empty
            # Add spaces between characters for char_tokenizer
        ...
        # Handle single sequence for ginkgo
        print("[DEBUG] it is not a list")
        text = ' '.join(list(text)) 
    ...
    if wrap:
      ...
      # map() above is called with batched=False, so `text` is a single string
      # and `toks` should be a flat List[int]. Return a flat list, not [[...]].
      if isinstance(toks[0], list):          # paranoia: if you ever switch to batched=True
          return {"input_ids": [ids + [EOS] for ids in toks]}
      else:
          return {"input_ids": toks + [EOS]}
  ...
  if streaming:
    ...
    tokenized_dataset = data.map(
      ...
      desc='Tokenizing')
  print("[DEBUG] tokenized_dataset, ", tokenized_dataset[0])
  if dataset_name == 'ptb':
  ...
  if not wrap:
    ...
    # Return dataset with torch format but ensure copies are made
    return tokenized_dataset.with_format('torch', columns=['input_ids', 'attention_mask', 'token_type_ids'], output_all_columns=True)
...
def get_tokenizer(config):
  ...
  elif config.data.tokenizer_name_or_path == 'kmer':
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        "Genereux-akotenou/BacteriaCDS-DNABERT-K6-89M")

  elif config.data.tokenizer_name_or_path == 'char_tokenizer':
    print("Loading char_tokenizer")
    # Use the original char_tokenizer that works with space-separated input
    tokenizer_dir = os.path.join(os.path.dirname(__file__), "char_tokenizer")
    tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_dir, local_files_only=True)
    print("Loaded char_tokenizer")
  else:
    tokenizer = transformers.AutoTokenizer.from_pretrained(
  ...
  # (optional but useful) print once:
  print("specials:",
...
def safe_collate_fn(batch):
  """Custom collate function that ensures tensors are cloned to avoid in-place modifications."""
  if isinstance(batch[0], dict):
    ...
    for key in batch[0].keys():
      ...
      if torch.is_tensor(values[0]):
        # Clone tensors to ensure they don't share memory
        collated[key] = torch.stack([v.clone() for v in values])
      else:
        collated[key] = torch.stack(values)
    return collated
  else:
    # Handle tensor batches
    return torch.stack([item.clone() if torch.is_tensor(item) else item for item in batch])

def get_dataloaders(config, tokenizer, skip_train=False,
                    skip_valid=False, valid_seed=None):
  num_gpus = torch.cuda.device_count()
  print(f"Number of GPUs: {num_gpus}")
  print(f"Number of nodes: {config.trainer.num_nodes}")
  print(f"Number of devices: {config.trainer.devices}")
  print(f"Accumulate grad batches: {config.trainer.accumulate_grad_batches}")
  print(f"Batch size: {config.loader.batch_size}")
  print(f"Eval batch size: {config.loader.eval_batch_size}")
  print(f"Global batch size: {config.loader.global_batch_size}")
  print(f"Eval global batch size: {config.loader.eval_global_batch_size}")
  ...
  if config.data.valid == 'same_as_train':
    # Use the same dataset for validation as training
    valid_set = train_set
  elif config.data.valid in ['text8', 'lm1b', 'ag_news', 'ncbi_hm', 'ginkgo', 'ginkgo_utr', 'ginkgo_3utr']:

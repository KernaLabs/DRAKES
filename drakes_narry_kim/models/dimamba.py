...
class DiMamba(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int, pad_token_id: int):
    ...
    if (getattr(config.training, 'guidance', None) is not None or
        (hasattr(config, 'guidance')
         and config.guidance is not None
         and getattr(config.guidance, 'method', None) == 'cfg')):
    ...
    # Scalar features conditioning (e.g., Vienna RNA scalars)
    if (hasattr(config, 'scalar_features')
        and config.scalar_features is not None
        and getattr(config.scalar_features, 'enabled', False)):
      self.scalar_map = ScalarEmbedder(
        config.scalar_features.num_features,
        config.model.cond_dim)
    else:
      self.scalar_map = None
    ...
    mamba_config = BiMambaConfig(
      ...
      use_adaLN=self.sigma_map is not None or self.cond_map is not None or self.scalar_map is not None,
  ...
  def forward(
    ...
    if scalar_features is not None:
      if self.scalar_map is None:
        raise ValueError("Scalar features provided, "
                         "but Model was not initialized "
                         "with scalar embedding layer.")
      else:
        c = c + F.silu(self.scalar_map(scalar_features)) if c is not None \
          else F.silu(self.scalar_map(scalar_features))
    if cond is not None:
...
class DiMambaClassifier(nn.Module):
  def __init__(self, config, vocab_size: int, pad_token_id: int):
    ...
    mamba_config = BiMambaConfig(
      ...
      use_adaLN=self.sigma_map is not None or self.scalar_map is not None,
  ...
  def forward(
    ...
    if self.sigma_map is not None:
      c = F.silu(self.sigma_map(sigma))
      if self._log_step < 5:
        print(f"\n[CHECKPOINT 2] Time conditioning (forward pass {self._log_step}):")
        print(f"  sigma: mean={sigma.mean():.4f}, std={sigma.std():.4f}, range=[{sigma.min():.4f}, {sigma.max():.4f}]")
        print(f"  cond c: mean={c.mean():.4f}, std={c.std():.4f}, range=[{c.min():.4f}, {c.max():.4f}]")
        print(f"  sigma_map.mlp[0].weight.norm: {self.sigma_map.mlp[0].weight.detach().norm():.4f}")
        print(f"  sigma_map.mlp[0].bias[:5]: {self.sigma_map.mlp[0].bias.detach()[:5].cpu().numpy()}")
    if scalar_features is not None:
      if self.scalar_map is None:
        raise ValueError("Scalar features provided, "
                         "but Classifier was not initialized "
                         "with scalar embedding layer.")
      else:
        c = c + F.silu(self.scalar_map(scalar_features)) if c is not None \
          else F.silu(self.scalar_map(scalar_features))
    ...
    if self._log_step < 5:
      with torch.no_grad():
        print(f"\n[CHECKPOINT 3] BiMamba hidden states (forward pass {self._log_step}):")
        print(f"  Shape: {x.shape}")
        print(f"  Mean: {x.mean():.6f}, Std: {x.std():.6f}, Range: [{x.min():.6f}, {x.max():.6f}]")
        pos_means = x.mean(dim=(0, 2))
        print(f"  Per-position mean range: [{pos_means.min():.6f}, {pos_means.max():.6f}], std: {pos_means.std():.6f}")
        feature_means = x.mean(dim=(0, 1))
        print(f"  Per-feature mean range (BEFORE pooling): [{feature_means.min():.6f}, {feature_means.max():.6f}], std: {feature_means.std():.6f}")
    ...
    if self.pooling == 'mean':
      ...
      seq_lengths = torch.sum(attention_mask, dim=1, keepdim=True) + 1e-15
      x = torch.sum(masked_x, dim=1) / seq_lengths
      if self._log_step < 5:
        with torch.no_grad():
          print(f"\n[CHECKPOINT 4] Pooled features (forward pass {self._log_step}):")
          print(f"  Pooled x shape: {x.shape}")
          print(f"  Mean: {x.mean():.6f}, Std: {x.std():.6f}, Range: [{x.min():.6f}, {x.max():.6f}]")
          sample_norms = x.norm(dim=1)
          print(f"  Per-sample L2 norms: mean={sample_norms.mean():.6f}, std={sample_norms.std():.6f}")
          print(f"  First 3 sample norms: {sample_norms[:3].cpu().numpy()}")
          print(f"  Seq lengths: mean={seq_lengths.mean():.1f}, range=[{seq_lengths.min():.0f}, {seq_lengths.max():.0f}]")
          feature_means = x.mean(dim=0)
          print(f"  Per-feature mean range: [{feature_means.min():.6f}, {feature_means.max():.6f}], std: {feature_means.std():.6f}")
    else:
      raise NotImplementedError(
    ...
    if self._log_step < 5:
      with torch.no_grad():
        pooled_features = x
        if self._log_step == 0:
          x_norm = F.normalize(pooled_features, dim=1)
          w_norm = F.normalize(self.output_layer.weight.detach(), dim=1)
          alignment = x_norm @ w_norm.T
          print(f"\n[CHECKPOINT 5] Output layer analysis (forward pass {self._log_step}):")
          print(f"  Feature-weight alignment (normalized): {alignment.mean(dim=0).cpu().numpy()}")

    x = self.output_layer(x)

    if self._log_step < 5:
      with torch.no_grad():
        print(f"\n[CHECKPOINT 5] Output layer logits (forward pass {self._log_step}):")
        print(f"  Logits shape: {x.shape}")
        print(f"  Mean: {x.mean():.6f}, Std: {x.std():.6f}")
        class_means = x.mean(dim=0)
        class_stds = x.std(dim=0)
        print(f"  Per-class logit means: {class_means.cpu().numpy()}")
        print(f"  Per-class logit stds: {class_stds.cpu().numpy()}")
        preds = x.argmax(dim=-1)
        unique_preds, pred_counts = torch.unique(preds, return_counts=True)
        print(f"  Pred distribution: {dict(zip(unique_preds.cpu().tolist(), pred_counts.cpu().tolist()))}")
        print(f"  Output layer weight shape: {self.output_layer.weight.shape}")
        print(f"  Weight mean: {self.output_layer.weight.mean():.6f}, std: {self.output_layer.weight.std():.6f}")
        print(f"  Per-class weight norms: {self.output_layer.weight.norm(dim=1).cpu().numpy()}")
        if self.output_layer.bias is not None:
          print(f"  Bias: {self.output_layer.bias.cpu().numpy()}")
        self._log_step += 1
